...
interface ComputedMapper<T> {
  <Key extends keyof T, Map extends Record<string, Key>>(map: Map): { [K in keyof Map]: () => T[Map[K] & Key] }
...
interface MethodsMapper<T, R> {
  <Key extends keyof T, Map extends Record<string, Key>>(map: Map): { [K in keyof Map]: (payload?: T[Map[K] & Key]) => R }
